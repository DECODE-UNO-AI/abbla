...
const SortableItem = sortableElement(
  ({
    ...
    const inputIndex = messageInputs[`message${messageIndex}Inputs`].findIndex(
      (input) => input.id === value.id
    );
    const input =
      messageInputs[`message${messageIndex}Inputs`][inputIndex] || null;
    return (
      <Box className={classes.inputContainer}>
        <Box
          className={classes.iconContainer}
          style={{ display: visualize ? "none" : "" }}
        >
          {value.type === "text" && setOpenIAModal ? (
            <Box
          ...
          >
            ...
            <button
              className={classes.closeButton}
              onClick={() => handleOnDeleteInput(value.id)}
            ></button>
          </Box>
        </Box>
        {value.type === "text" ? (
          ...
          <>
            {input.value ? (
              <Box
                ...
                style={{ cursor: visualize ? "auto" : "move" }}
              >
...
const DynamicInputs = ({
  ...
  const currentInputOrder =
    inputOrder[`message${messageIndex}InputOrder`] || [];
  const currentMessageInputs =
    messageInputs[`message${messageIndex}Inputs`] || [];

  const handleAddInput = () => {
    setMessageInputs((e) => ({
      ...e,
      [`message${messageIndex}Inputs`]: [
        ...currentMessageInputs,
        { id: currentMessageInputs.length, type: "text", value: "" },
      ],
    }));
    setInputOrder((e) => ({
      ...e,
      [`message${messageIndex}InputOrder`]: [
        ...currentInputOrder,
        currentMessageInputs.length,
      ],
    }));
  };
  ...
  const handleAddInputFile = () => {
    setMessageInputs((e) => ({
      ...
      [`message${messageIndex}Inputs`]: [
        ...currentMessageInputs,
        { id: currentMessageInputs.length, type: "file", value: "" },
      ],
    }));
    setInputOrder((e) => ({
      ...e,
  ...
  const handleOnDeleteInput = (inputIndex) => {
    ...
    const orderIndex = inputOrder[`message${messageIndex}InputOrder`].findIndex(
      (i) => i === inputIndex
    );
  ...
  const onSortEnd = ({ oldIndex, newIndex }) => {
    setInputOrder((e) => ({
      ...e,
      [`message${messageIndex}InputOrder`]: arrayMoveImmutable(
        currentInputOrder,
        oldIndex,
        newIndex
      ),
  ...
  return (
    <>
      <SortableContainer
        onSortEnd={onSortEnd}
        distance={visualize ? 99999999999 : 1}
      >
        {currentInputOrder.map((inputId, index) => {
          const input = currentMessageInputs.find(
          ...
          {visualize ? (
            ...
            <>
              <Button type="button" onClick={handleAddInput}>
                +{i18n.t("campaignModal.form.addText")}
              </Button>
              <Button type="button" onClick={handleAddInputFile}>
                +{i18n.t("campaignModal.form.addFile")}
              </Button>
            </>
          )}
        </Box>
