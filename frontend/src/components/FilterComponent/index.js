...
import { Button, makeStyles, Paper, InputLabel } from "@material-ui/core";

import { Tune, CloseSharp } from "@material-ui/icons";
...
const FilterComponent = ({
  ...
  useEffect(() => {
    const departaments = user?.departaments?.map((dep) => {
  ...
  useEffect(() => {
    api.get("/users/", {}).then((data) => {
      const aten = data.data.users.map((e) => {
  ...
  useEffect(() => {
    api.get("/whatsapp/").then((data) => {
      const cons = data.data.map((con) => {
        return { value: con.id, label: con.name };
      });
      setConnections(cons);
    });
  ...
  const handleOnDepartamentChange = (e) => {
    ...
    const currentDepartaments = user?.departaments.filter((dep) =>
      e.includes(`${dep.id}`)
    );
    let departamentsQueues = [];
    currentDepartaments.map(
      (dep) => (departamentsQueues = [...departamentsQueues, ...dep.queues])
    );
    const queueFilterValues = departamentsQueues.map((queue) => {
      return { value: `${queue.id}`, label: queue.name };
    });
    // eslint-disable-next-line no-sequences
    const uniqueQueueFilterValues = queueFilterValues
    ...
    <>
      <Paper className={classes.container}>
        <Tune
          onClick={() => {
            setIsModalOpen(true);
            setWidth && setWidth("600px");
            setFilterModalOpen && setFilterModalOpen(true);
          }}
        ...
        {isModalOpen ? (
          <Paper
            style={{
              ...
              top: 0,
              right: 0,
              width: calledByGroupCreator ? "600px" : "100%",
              height: calledByGroupCreator ? "651px" : "100%",
              minHeight: calledByGroupCreator ? 651 : 300,
              boxShadow: calledByGroupCreator
                ? "unset"
          ...
          >
            ...
            {status !== "search" ? (
              <div className={classes.filterOption}>
                ...
                <TagsFilter
                  style={{ padding: 0 }}
                  onFiltered={handleOnTagsChange}
                  selecteds={selectedsTags}
                  setSelecteds={setSelectedsTags}
                />
              </div>
            ) : (
              ""
