...
function TabPanel(props) {
  ...
  return (
    <Paper
      ...
      {...other}
    >
      {value === index && <div style={{ marginTop: 15 }}>{children}</div>}
    </Paper>
  );
}
...
const Connections = () => {
  ...
  const handleDeleteWhatsAppSession = async (whatsAppId) => {
    try {
    ...
    if (confirmModalInfo.action === "disconnect") {
      try {
        if (confirmModalInfo.isApi) {
          await api.delete(
            `/whatsappapisession/desconect/${confirmModalInfo.whatsAppId}`
          );
          return;
        }
        ...
        {whatsApp.status === "qrcode" && (
          <>
            ...
            <Button
              ...
              variant="outlined"
              color="secondary"
              onClick={() => handleDeleteWhatsAppSession(whatsApp.id)}
            >
              Pausar conexão
            </Button>
          </>
      ...
      <div className={classes.customTableCell}>
        ...
        {whatsApp.status === "PAUSED" && (
          <CustomToolTip
            ...
            content={
              "Conexão pausada, entre em contato com o suporte para mais informações"
            }
          >
            <PauseCircleFilled />
          </CustomToolTip>
